...
using Lcgoc.Weixin.SDK.Entities;
...
namespace Lcgoc.Web
{
    public class WeixinExecutor : IWeixinExecutor
    {
        ...
        public string Execute(WeixinMessage message, string decryptMsg, string guid)
        {
            var result = "";
            ...
            crm_weixin_log log = new crm_weixin_log() { guid = guid, domain = domain, openId = openId, myUserName = myUserName, request = decryptMsg, Type = message.Type.ToString(), startTime = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff") };
            ...
            if (Lcgoc.Common.ConfigHelper.GetConfigString("dblog") == "1")
                new Thread((ThreadStart)delegate { bll.WriteWeixinLog(log); }).Start();
            return result;
        }
    }
}